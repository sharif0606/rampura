...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('initial_stock_details', function (Blueprint $table) {
            $table->id();
            $table->unsignedBigInteger('company_id')->index()->foreign()->references('id')->on('companies')->onDelete('cascade');
            $table->unsignedBigInteger('initial_stock_id')->nullable()->index()->foreign()->references('id')->on('initial_stocks')->onDelete('cascade');
    ...
    {
        Schema::dropIfExists('initial_stock_details');
    }
};
